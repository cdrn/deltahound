...
import { Token, Pool, Route, Trade, TradeType } from '@uniswap/v3-sdk';
import { CurrencyAmount, TradeType as SDKTradeType, Percent } from '@uniswap/sdk-core';
...
export class UniswapConnector implements VenueConnector {
  ...
  private readonly tokenMap: { [key: string]: string } = {
    ...
    'WETH': WETH_ADDRESS,
    'USDT': USDT_ADDRESS,
    'USDC': '0xA0b86a33E6412E4a7c2F6e8b7d0A3d6E9c7A1A2b'
  };
  ...
  private getTokenAddress(symbol: string): string {
    const address = this.tokenMap[symbol.toUpperCase()];
    if (!address) {
  ...
  async getPriceData(baseSymbol: string, quoteSymbol: string): Promise<PriceData> {
    try {
      const tokenInAddress = this.getTokenAddress(baseSymbol);
      const tokenOutAddress = this.getTokenAddress(quoteSymbol);
      ...
      const price = parseFloat(amountOut.toString()) / 1e18;
